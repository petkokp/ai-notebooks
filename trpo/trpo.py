...
import numpy as np
from utils import explained_variance, categorical_kl, get_flat_params_from, set_flat_params_to
...
class TRPO:
    def __init__(self, **config):
        ...
        self.model = CNNModel(
            self.config["state_shape"], self.config["n_actions"]).to(self.device)
        self.mse_loss = torch.nn.MSELoss()
        self.value_optimizer = Adam(
            self.model.critic.parameters(), self.config["value_lr"])
    ...
    def train(self, states, actions, rewards, dones, log_probs, probs, values, next_values):
        returns = self.get_returns(
            rewards, values, next_values, dones, n=self.config["n_workers"])
        ...
        def get_actor_loss() -> dict:
            ...
            a_loss = -torch.mean((log_prob - old_log_prob).exp()
                                 * advs) - self.config["ent_coeff"] * ent
    ...
    def optimize_actor(self, loss_dict: dict, loss_fn):
        ...
        def fisher_vector_product(y):
            ...
            grads = torch.autograd.grad(
                kl, self.model.actor.parameters(), create_graph=True)
            flat_grads = torch.cat([g.view(-1) for g in grads])

            inner_prod = flat_grads.t() @ y
            grads = torch.autograd.grad(
                inner_prod, self.model.actor.parameters())
            flat_grads = torch.cat([g.reshape(-1) for g in grads]).data
            return flat_grads + y * self.config["damping"]

        opt_dir = conjugate_gradient(fisher_vector_product, -j, self.config["k"])
        quadratic_term = (opt_dir * fisher_vector_product(opt_dir)).sum()
        beta = torch.sqrt(
            2 * self.config["trust_region"] / (quadratic_term + 1e-6))
    ...
    def get_returns(self,
        ...
        for worker in range(n):
            extended_values[worker] = np.append(
                values[worker], next_values[worker])
            gae = 0
